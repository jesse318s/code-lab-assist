...
### 6. `lifespan` Context Manager — Lifecycle Events

Startup and shutdown logic lives in a single `@asynccontextmanager` function passed to `FastAPI(lifespan=)`. Code before `yield` runs on startup; code after `yield` runs on shutdown. This replaces the deprecated `@app.on_event` decorator (removed in FastAPI 0.93+).
...
async def lifespan(app: FastAPI):
    ...
    # Startup: initialize config, event bus, exchange adapters, etc.
